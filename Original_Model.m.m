...
for i=1:n
    if A(i)>=1 && A(i)<=10
        service_time(i)=1;
    elseif A(i)>=11 && A(i)<=30
        service_time(i)=2;
    elseif A(i)>=31 && A(i)<=60
        service_time(i)=3;
    elseif A(i)>=61 && A(i)<=85
        service_time(i)=4;
    elseif A(i)>=86 && A(i)<=95
        service_time(i)=5;
    else
        service_time(i)=6;
    end
...
service_end_time=zeros(n,1);        % ✅ MOVED UP

service_start_time(1)=arrival_time(1);
service_end_time(1)=service_start_time(1)+service_time(1);   % ✅ MOVED UP

for i=2:n
    service_start_time(i)=max(arrival_time(i),service_end_time(i-1));
    service_end_time(i)=service_start_time(i)+service_time(i);   % ✅ MOVED UP
end
...
array2table(results,'VariableNames',header)
